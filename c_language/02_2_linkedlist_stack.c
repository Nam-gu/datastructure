...
//pop : 데이터 추출
void pop(Node *s){
	Node *p = finalNode(s); //맨 마지막 노드 검색
	if(p->L_link == NULL) {
		isListEmpty(s);
		return;
	}
	printf("pop : %c\n",p->data); //데이터 출력
	p->L_link->R_link = NULL; //마지막 노드 전의 노드의 오른쪽 링크 NULL로 비워준다.
	//NULL로 비워주는 이유는 R_link가 NULL이어야 해당 노드가 마지막임을 알 수 있다. 
	free(p); // 노드 삭제 
} 
...
void main() {
	...
	printf("***********Command***********\n");
  	printf("+<c> : add <c>, P : delete <c>, S : show all stack, R : show reverse stack\n");
